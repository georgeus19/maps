#ifndef BACKEND_ADJACENCY_GRAPH_VECTOR_H
#define BACKEND_ADJACENCY_GRAPH_VECTOR_H
...
template <typename V, typename E>
AdjacencyListGraphVector<V, E>::AdjacencyListGraphVector() : g_() {
...
template <typename V, typename E>
inline void AdjacencyListGraphVector<V, E>::AddEdge(E && edge) {
...
template <typename V, typename E>
inline V& AdjacencyListGraphVector<V, E>::GetVertex(unsigned_id_type id) {
    assert(id < g_.size());
    return g_[id];
}

template <typename V, typename E>
void AdjacencyListGraphVector<V, E>::ForEachVertex(const std::function<void(V&)>& f) {
    for (auto&& vertex : g_) {
        f(vertex);
    }
}
...
template <typename V, typename E>
void AdjacencyListGraphVector<V, E>::ForEachEdge(const std::function<void(E&)>& f) {
...
template <typename V, typename E>
void AdjacencyListGraphVector<V, E>::AddEdge(E && e, const std::function<void(V &, E && e)>& add_edge) {
...
template <typename V, typename E>
size_t AdjacencyListGraphVector<V, E>::GetVertexCount() const {
    return g_.size();
}

template <typename V, typename E>
size_t AdjacencyListGraphVector<V, E>::GetEdgeCount() {
    size_t count = 0;
    ForEachEdge([&](E&){
        ++count;
    });
...
#endif //BACKEND_ADJACENCY_GRAPH_VECTOR_H